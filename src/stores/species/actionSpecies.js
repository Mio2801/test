...
import { ApiGetAllSpecies, UpdateSpecies, apiGetClass, apiGetFamily, apiGetGenus, apiGetKingdom, apiGetOrder, apiGetPhylum, createSpecies, deleteSpecies } from "../../api/auth/auth.api";

export const getSpeciesAction = createAsyncThunk("species", async ({ page, itemsPerPage, search }) => {
	try {
		const response = await ApiGetAllSpecies(page, itemsPerPage, search);
		return response;
	} catch (error) {
		throw error;
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
// giới kingdom
export const getKingdomAction = createAsyncThunk("kingdom", async () => {
	try {
		const response = await apiGetKingdom();
		return response;	
	} catch (error) {
		throw error;
	}
...
export const getKingdomReduces = (builder) => {
	builder
		.addCase(getKingdomAction.pending, (state) => {
			state.loadingKingdom = true;
			state.error = null;
		})
		.addCase(getKingdomAction.fulfilled, (state, action) => {
			state.loadingKingdom = false;
			state.dataKingdom = action.payload;
			state.error = null;
		})
		.addCase(getKingdomAction.rejected, (state, action) => {
			state.loadingKingdom = false;
			state.error = action.error.message;
		});
}
