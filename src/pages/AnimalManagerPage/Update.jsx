...
import { getPhylumAction, getSpeciesAction, getKingdomAction, getClassAction, getOrderAction, getFamilyAction, getGenusAction, UpdateSpeciesAction } from "../../stores/species/actionSpecies"
...
const UpdateSpecies = (item, searchInput) => {
	...
	useEffect(() => {
		setDataUpdate({
			...
			// kingdom_id: item.kingdom.ten_khoa_hoc,
			// phylum_id: item.phylum.ten_khoa_hoc,
            // class_id: item.class.ten_khoa_hoc,
            // order_id: item.order.ten_khoa_hoc,
            // family_id: item.family.ten_khoa_hoc,
            // genus_id: item.genus.ten_khoa_hoc,
		});
	}, [item]);
	...
	const handelupdateSpecies = (id) => {
		const validationErrors = validateForm(dataUpdate);
		if (validationErrors) {
			alert("Cập Nhật Người dùng thành công");
            navigate("/auth")
			setErrors({});
			dispatch(UpdateSpeciesAction({ id: id, data: dataUpdate, kingdom_id: allKingdom, phylum_id: allPhylum, class_id: allClass, order_id: allOrder, family_id: allFamily, genus_id: allGenus }))
				.then(() => {
					dispatch(getSpeciesAction({ page: pagination.page, perpage: pagination.itemsPerPage, input: searchInput}));
				})
		} else {
            ...
            <div className="form_species">
                ...
                <div style={{width:'1000px'}}>
                    ...
                    <div className="form_info w-100">
                        ...
                        <input className="input_form" type="text" id='ten' placeholder="Tên" onChange={handelupdate} />
                    ...
                    <div className="d-flex w-80">
                        <div className="form_info w-50">
                            ...
                            <input className="input_form" type="text" id='ten_khoa_hoc' placeholder="Tên khoa học" onChange={handelupdate} />
                            {errors.science_name !== "" && (<span className="error">{errors.science_name}</span>)}
                        </div>
                        <div className="form_info w-50">
                            <label htmlFor="">Tên tác giả</label><br />
                            <input className="input_form" type="text" id='ten_tac_gia' placeholder="Tên tác giả" onChange={handelupdate} />
                        </div>
                    </div>
                    <div className="form_info w-100">
                        <label htmlFor="">Tên địa phương</label><br />
                        <input className="input_form" type="text" id='ten_dia_phuong' placeholder="Tên địa phương" onChange={handelupdate} />
                    </div>
                    <div className="form_info w-100">
                        <label htmlFor="">Nguồn dữ liệu</label><br />
                        <input className="input_form" type="text" id='nguon_du_lieu' placeholder="Nguồn dữ liệu" onChange={handelupdate} />
                    </div>
                </div>
